<?php
    ...
    echo "==== Start Proccess Trade Altmarkets\n";
...
function doAltMarketsTrading()
{
	...
	echo ("Check Balance\n");
	$balances = altmarkets_api_get('account/balances');
    // print_r ($balances);
		...
		else
		{
			...
			#if ($amount>1)
			{
				...
				if ($result->num_rows>0)
				{
			        ...
			        $ticker = altmarkets_api_query("markets/$uri/tickers");
			        ...
			        if ($limit_price>$min_sell_price)
			        {
			            ...
			            print_r ($params);
			            $res = altmarkets_api_post('market/orders', $params);
			            print_r ($res);
			...
			if ($result_market->num_rows>0)
			{
				...
				// if ($market->deposit_address=="")
				{
					...
					$res = altmarkets_api_get('account/deposit_address/' .strtolower($symbol));
					// print_r ($res);
					$address = $res['address'];
...
function altmarkets_api_get($method, $req = array())
{
    // echo __METHOD__;
    require_once('/etc/yiimp/keys.php');
    $uri = "https://v2.altmarkets.io/api/v2/peatio/$method";

    $reqStr=print_r($req, true);
    sleep(1);

    // optional secret key
    if (empty(EXCH_ALTMARKETS_SECRET) && strpos($method, 'public') === FALSE) return FALSE;
    if (empty(EXCH_ALTMARKETS_KEY) && strpos($method, 'public') === FALSE) return FALSE;
 ...
 //    if (GRAVIEX_PROXY_IP)
	// {
 //    	curl_setopt($ch, CURLOPT_PROXY, GRAVIEX_PROXY_IP);
 ...
 //    }
    ...
    // $header = substr($execResult, 0, $header_size);
    // $body = substr($execResult, $header_size);
...
function altmarkets_api_post($method, $postData = array())
{
    // echo __METHOD__;
    require_once('/etc/yiimp/keys.php');
    $uri = "https://v2.altmarkets.io/api/v2/peatio/$method";
...
function auth() {
    ...
    $apikey = EXCH_ALTMARKETS_KEY; // your API-key
    $apisecret = EXCH_ALTMARKETS_SECRET; // your Secret-key
